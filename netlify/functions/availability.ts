// Netlify Function — proxy the read-only OCTO /availability endpoint.
//
// POST body: { productId, optionId?, localDateStart, localDateEnd }
// Every availability request needs a productId and an optionId; where a product has
// no options, use "DEFAULT" (this function defaults optionId to DEFAULT if omitted).
//
// The OCTO connection key (VENTRATA_OCTO_KEY) is server-side only — never in the DOM.
// Cache: micro-cache 1 minute with stale-while-revalidate up to 3 minutes, to absorb
...
export default withGuard(async (request: Request): Promise<Response> => {
  ...
  try {
    const upstream = await fetch(`${OCTO_BASE}/availability`, {
      ...
      // empirically confirmed; the API rejects localDateTimeStart/localDateTimeEnd (those
      // are RESPONSE slot fields, not request params). No time expansion needed.
